...
int main(int argc, char *argv[])
{
  ...
  char *op_beq   = "00000111"; //7
    ...
    char *op_mult  = "00001000"; // 8
    char *op_sll   = "00001001"; // 9
    char *op_srl   = "00001010"; // 10
    char *op_sra   = "00001011"; // 11
    char *op_ror   = "00001100"; // 12
    char *op_bne   = "00001101"; // 13
    // pushed mem instructions down to avoid conflicts
    ...
    while (fgets(line, LINE_SIZE, fi) != NULL) //Read a line from the input .s file
  {
      ...
      in_token = strtok(tline, delim); // read the first token
        ...
        else //the line is either not an inst, or an inst with all four tokens
            strcpy(pline, line);
      ...
      while (in_token != NULL)
	    {
            ...
            // Handling lines/words which are not part of an inst
            else
          {
              count = 99;
                break;
      ...
      if (count == 4) //line contains a valid inst
          fputs("\n", fo);
	    else if (count != 0) //line is neither a valid inst, nor a valid comment / empty line
        {
