// Control unit module - Decodes instructions and generates the  required control signals
// LAB 4: Updated to support j and beq  instructions
// LAB 4.5: Extended the ISA to include mult, sll, srl, sra, ror, and bne instructions
...
module control_unit(
    ...
    output reg READ_MEM,          // NEW: memory read enable
    output reg WRITE_MEM,         // NEW: memory write enable
    output reg MUX_MEM_SEL        // NEW: selects memory readdata for register write
);
    ...
    // Extract register read adresses immediately (0 delay)
    // This enables the register file reading to execute in parralel with decoding.
    // Required to meet the  strict timing constraints.
    ...
    always @(INSTRUCTION) begin
        ...
        // Intialize default values for all signals 
        // to prevent unintended latch inferences during  synthesis
        WRITEENABLE  = 1'b0;
        MUX_IMM_SEL  = 1'b0;
        MUX_COMP_SEL = 1'b0;
        ALUOP        = 3'b000;
        ...
        READ_MEM     = 1'b0;      // NEW: default no memory read
        WRITE_MEM    = 1'b0;      // NEW: default no memory write
        MUX_MEM_SEL  = 1'b0;      // NEW: default use ALU result
        ...
        case (OPCODE)
            ...
            // sub instruction - utilizes the adder with 2s complement conversion
            // opcode = 0x03
            8'b00000011: begin
            ...
            // =============================================
            // LAB 4 INSTRUCTIONS - FLOW CONTROL
            // =============================================

            // j (jump) - Unconditional jump instruction
            // opcode = 0x06
            // Format: j offset (offset is located in bits[23:16] and treated as signed)
            // Target address = PC_NEXT + (offset * 4), where PC_NEXT = PC + 4
            // Bits [15:0] are not utilized
            8'b00000110: begin
            ...
            // =============================================
            // LAB 4.5 INSTRUCTIONS - EXTENDED ISA
            // =============================================
            ...
            // sra (shift right arithmetic) - shift right while preserving the sign bit
            // opcode = 0x0B
            // Format: sra rd, rt, imm
            // reg[rd] = reg[rt] >>> imm (fills with the sign bit from the  left)
            8'b00001011: begin
                ALUOP        = 3'b100;   // select shift unit
            ...
            // =============================================
            // NEW INSTRUCTIONS FOR LAB 5 - DATA MEMORY
            // =============================================
            
            // lwd (load word direct) - memory -> register
            // opcode = 0x0E
            8'b00001110: begin
                ALUOP        = 3'b000;   // forward RS (address)
                MUX_IMM_SEL  = 1'b0;     // use RS
                MUX_COMP_SEL = 1'b0;
                WRITEENABLE  = 1'b1;     // write to RD
                READ_MEM     = 1'b1;     // read from memory
                WRITE_MEM    = 1'b0;
                MUX_MEM_SEL  = 1'b1;     // select memory data to write back
            end

            // lwi (load word immediate) - memory -> register
            // opcode = 0x0F
            8'b00001111: begin
                ALUOP        = 3'b000;   // forward IMM (address)
                MUX_IMM_SEL  = 1'b1;     // use IMM
                MUX_COMP_SEL = 1'b0;
                WRITEENABLE  = 1'b1;     // write to RD
                READ_MEM     = 1'b1;     // read from memory
                WRITE_MEM    = 1'b0;
            ...
            // swd (store word direct) - register -> memory
            // opcode = 0x10
            8'b00010000: begin
                ALUOP        = 3'b000;   // forward RS (address)
                MUX_IMM_SEL  = 1'b0;     // use RS
                MUX_COMP_SEL = 1'b0;
                WRITEENABLE  = 1'b0;     // don't write to register
                READ_MEM     = 1'b0;
                WRITE_MEM    = 1'b1;     // write to memory
                MUX_MEM_SEL  = 1'b0;
            end

            // swi (store word immediate) - register -> memory
            // opcode = 0x11
            8'b00010001: begin
                ALUOP        = 3'b000;   // forward IMM (address)
                MUX_IMM_SEL  = 1'b1;     // use IMM
            ...
            // Unrecognized opcode - disable writes to prevent state  corruption
            default: begin
                WRITEENABLE = 1'b0;
            end
        endcase
